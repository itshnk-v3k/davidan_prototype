...
- **Customer app**: a hub with DaviDan's five brands as bubbles (Restaurant, Sushi, Patiserie, Apă naturală, Rent Car) and the tabs Acasă, Comenzi, Favorite and Profil; demo sign-in (no real SMS), location selection with "use my current location". A brand opens full screen above the tabs. The bakery (Patiserie) and Sushi have their menu, product, cart, checkout and order tracking, and Sushi also has an information page with its contacts and legal pages. Apă naturală is one page with its two 0,5L bottles and **Comandă din nou**, which puts the last water order back in the cart. Rent Car lists davidanrentcar.md's 11 cars; each car's page has its specs, every price per day, the location fee and its insurance amount, and a **Cerere de rezervare** form that prices the rental line by line and sends a request (nothing is paid; "Vă vom contacta în curând."). Sent requests show in Comenzi and on the hub next to orders, and Rent Car has an information page with its contacts and legal pages. Restaurant shows "În curând" (the client has no menu).
- **Carts**: each brand has its own cart and checkout. The receipt button on Acasă and Favorite lists every cart that has something in it. Sushi and water orders are delivered only: no source names a place to pick them up. A car rental is a request, not a cart.
- **Languages**: Română, the default, and Русский, chosen in Profil (or **Ca telefonul**, which follows the phone). Everything switches at once, menus and brand pages included, except the brands' legal pages, which exist only in Romanian and say so in Russian. Only the sushi menu has Russian from its site; the rest is our translation, listed for DaviDan's review in `docs/sources/ru_translations.md`.
...
- Flutter's gen-l10n (`flutter_localizations` + [intl](https://pub.dev/packages/intl)) for the UI text in Romanian, the default, and Russian: `lib/l10n/app_ro.arb` and `app_ru.arb`, from which `flutter pub get`, `run` and `build` regenerate `lib/l10n/app_localizations*.dart`. Setup and gotchas: `docs/sources/flutter_l10n.md`
- Content (menus, shops, the fleet, brand pages) is written in Romanian in `lib/data/mock/`, with its Russian in `lib/data/mock/ru/`, keyed by the Romanian it translates. The catalog, shop and fleet providers hand screens the content in the app's language (`lib/l10n/content.dart`), as a backend would for `?lang=ru`.

...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      final kurtosCount = container
          .read(productsByCategoryProvider(CategoryIds.kurtos))
          .length;
      await pumpApp(tester, container, Routes.clientHome);
      await scrollHomeTo(tester, row(CategoryIds.kurtos));

      await tapVisible(
        tester,
        inRow(CategoryIds.kurtos, find.text(AppStrings.seeAll)),
      );
      ...
      await tester.scrollUntilVisible(
        endTile,
        150,
        scrollable: rowScroll(CategoryIds.kurtos),
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(container.read(cartQuantitiesProvider), {
        'croissant-ciocolata': 1,
      });

      await scrollHomeTo(tester, row(CategoryIds.patiserie));
      final patiserieCard = cardInRow(CategoryIds.patiserie, croissant);
      await tester.scrollUntilVisible(
        patiserieCard,
        150,
        scrollable: rowScroll(CategoryIds.patiserie),
      );
      expect(
        find.descendant(
          of: patiserieCard,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        ...
        ProductImage.heroTagFor('croissant-ciocolata', scope: 'popular'),
      );
  ...
  testWidgets('every row fits a 360 px phone, cards with a stepper included', (
    ...
    for (final product in container.read(productsProvider)) {
